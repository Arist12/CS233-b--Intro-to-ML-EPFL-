...
def main(args):
    ...
    if args.use_pca:
        ...
        print(
            f"The total variance explained by the first {args.pca_d} principal components is {pca_obj.find_principal_components(xtrain):.3f} %"
        )
        # perform dimension reduction on input data
        xtrain, xtest = pca_obj.reduce_dimension(xtrain), pca_obj.reduce_dimension(
            xtest
        )
    ...
    if args.method == "nn":
        ...
        if args.nn_type == "mlp":
            ...
            cnn = False
            model = MLP(
                input_size=xtrain.shape[-1], n_classes=n_classes
            )  ### WRITE YOUR CODE HERE
